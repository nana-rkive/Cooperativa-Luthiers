...
describe('UsuarioService', () => {
  ...
  describe('cadastro()', () => {
    it('deve criar um usuário e retornar mensagem de sucesso', async () => {
      ...
      const result = await service.cadastro(
        'Maria',
        'Souza',
        'maria@example.com',
        'senha123',
    ...
    it('AUTH_001 – deve lançar exceção quando e-mail já está cadastrado', async () => {
      ...
      await expect(
        service.cadastro('Maria', 'Souza', 'maria@example.com', 'senha123'),
  ...
  describe('ativarConta()', () => {
    it('deve ativar a conta com token válido', async () => {
      const inativo = makeUsuario({
        ativo: false,
        tokenAtivacao: 'valid-token',
      });
    ...
    it('AUTH_007 – deve lançar exceção quando token for inválido', async () => {
      ...
      await expect(service.ativarConta('invalid-token')).rejects.toMatchObject({
        code: 'AUTH_007',
    ...
    it('AUTH_008 – deve lançar exceção quando conta já está ativa', async () => {
      ...
      await expect(service.ativarConta('some-token')).rejects.toMatchObject({
        code: 'AUTH_008',
        status: HttpStatus.CONFLICT,
      });
    });
  ...
  describe('login()', () => {
    ...
    it('AUTH_002 – deve lançar exceção quando e-mail não está cadastrado', async () => {
      repo.findByEmail.mockResolvedValue(null);
      await expect(
        service.login('desconhecido@example.com', 'senha'),
    ...
    it('AUTH_003 – deve lançar exceção quando senha for incorreta', async () => {
      ...
      await expect(
        service.login('maria@example.com', 'senha_errada'),
    ...
    it('AUTH_009 – deve lançar exceção quando conta não está ativa', async () => {
      ...
      await expect(
        service.login('maria@example.com', 'senha123'),
      ).rejects.toMatchObject({
        code: 'AUTH_009',
  ...
  describe('validarEmail()', () => {
    ...
    it('AUTH_002 – deve lançar exceção quando e-mail não encontrado', async () => {
      repo.findByEmail.mockResolvedValue(null);
      await expect(service.validarEmail('x@x.com')).rejects.toMatchObject({
        code: 'AUTH_002',
      });
    });
  });
  ...
  describe('redefinirSenha()', () => {
    ...
    it('AUTH_004 – deve lançar exceção quando senhas não coincidem', async () => {
      await expect(
        service.redefinirSenha('maria@example.com', 'nova123', 'diferente'),
    ...
    it('AUTH_002 – deve lançar exceção quando e-mail não existe', async () => {
      repo.findByEmail.mockResolvedValue(null);
      await expect(
        service.redefinirSenha('x@x.com', 'nova123', 'nova123'),
      ).rejects.toMatchObject({ code: 'AUTH_002' });
    });
  });
  ...
  describe('atualizar()', () => {
    ...
    it('AUTH_005 – deve lançar exceção quando usuário não existe', async () => {
      repo.findById.mockResolvedValue(null);
      await expect(
        service.atualizar(99, { primeiroNome: 'Joana' }),
    ...
    it('AUTH_006 – deve lançar exceção quando novo e-mail já está em uso', async () => {
      ...
      await expect(
        service.atualizar(1, { email: 'novo@email.com' }),
      ).rejects.toMatchObject({
        code: 'AUTH_006',
        status: HttpStatus.CONFLICT,
      });
  ...
  describe('deletar()', () => {
    ...
    it('AUTH_005 – deve lançar exceção quando usuário não existe', async () => {
      repo.findById.mockResolvedValue(null);
      await expect(service.deletar(99)).rejects.toMatchObject({
        code: 'AUTH_005',
      });
    });
  });
