...
export class InstrumentoController {
  ...
  @Roles('admin', 'luthier')
  @ApiOperation({
    summary: 'Registre a entrada de um instrumento na oficina para reparo',
  })
  @ApiBody({ type: CreateInstrumentoDto })
  @ApiResponse({
    status: 201,
    description: 'Instrumento registrado com sucesso.',
  })
  @ApiResponse({ status: 401, description: 'Não autenticado.' })
  @ApiResponse({ status: 403, description: 'Sem permissão.' })
  create(@Body() dto: CreateInstrumentoDto) {
    return this.instrumentoService.create(
      dto.modeloMadeira,
      dto.dataEntrada,
      dto.reparoConcluido,
      dto.custoReparo,
  ...
  update(@Param('id') id: string, @Body() dto: CreateInstrumentoDto) {
    return this.instrumentoService.update(
      Number(id),
      dto.modeloMadeira,
      dto.dataEntrada,
      dto.reparoConcluido,
  ...
  @Get()
  @ApiOperation({
    summary: 'Lista todos os instrumentos em manutenção na oficina',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Instrumento removido com sucesso.',
  })
  @ApiResponse({ status: 401, description: 'Não autenticado.' })
  @ApiResponse({
    status: 403,
