...
describe('InstrumentoService', () => {
    ...
    describe('create()', () => {
        ...
        beforeEach(() => {
            luthierRepo.findById.mockResolvedValue(makeLuthier({ dataAbertura: luthierAbertura }));
        ...
        it('deve criar instrumento com dados válidos', async () => {
            ...
            const result = await service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, p.custoReparo, p.luthierId);
        ...
        it('INSTRUMENTO_001 – deve lançar exceção quando luthier não existe', async () => {
            ...
            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, p.custoReparo, p.luthierId))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_001', status: HttpStatus.NOT_FOUND });
        });
        ...
        it('INSTRUMENTO_002 – deve lançar exceção quando dataEntrada for anterior à abertura da oficina', async () => {
            ...
            await expect(service.create(p.modeloMadeira, anteriorAbertura, p.reparoConcluido, p.custoReparo, p.luthierId))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_002' });
        });

        it('INSTRUMENTO_003 – deve lançar exceção quando custo for negativo', async () => {
            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, -1, p.luthierId))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_003' });
        });

        it('INSTRUMENTO_003 – deve lançar exceção quando custo for maior que 50000', async () => {
            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, 50001, p.luthierId))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_003' });
        });

        it('INSTRUMENTO_004 – deve lançar exceção quando reparo concluído mas custo é 0', async () => {
            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, true, 0, p.luthierId))
        ...
        it('INSTRUMENTO_005 – deve lançar exceção quando há duplicidade de modelo em reparo para o mesmo luthier', async () => {
            const duplicado = makeInstrumento({ modeloMadeira: 'Violão', reparoConcluido: false, luthierId: 1 });
            instrumentoRepo.findAll.mockResolvedValue([duplicado]);

            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, p.custoReparo, p.luthierId))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_005', status: HttpStatus.CONFLICT });
        });

        it('deve permitir criar mesmo modelo se o anterior já está concluído (reparoConcluido=true)', async () => {
            const concluido = makeInstrumento({ modeloMadeira: 'Violão', reparoConcluido: true, luthierId: 1 });
        ...
        it('deve permitir mesmo modelo em reparo para luthiers diferentes', async () => {
            const outroLuthier = makeInstrumento({ modeloMadeira: 'Violão', reparoConcluido: false, luthierId: 99 });
            instrumentoRepo.findAll.mockResolvedValue([outroLuthier]);
            instrumentoRepo.create.mockResolvedValue(makeInstrumento());

            const p = defaultParams();
            await expect(service.create(p.modeloMadeira, p.dataEntrada, p.reparoConcluido, p.custoReparo, 1))
                .resolves.toBeDefined();
        });
    });
    ...
    describe('findById()', () => {
        ...
        it('INSTRUMENTO_006 – deve lançar exceção quando não encontrado', async () => {
            ...
            await expect(service.findById(99))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_006', status: HttpStatus.NOT_FOUND });
        });
    });
    ...
    describe('deactivate()', () => {
        ...
        it('INSTRUMENTO_006 – deve lançar exceção quando não encontrado', async () => {
            instrumentoRepo.findById.mockResolvedValue(null);
            await expect(service.deactivate(99))
                .rejects.toMatchObject({ code: 'INSTRUMENTO_006' });
        });
    });
