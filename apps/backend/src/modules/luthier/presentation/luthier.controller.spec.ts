...
describe('LuthierController', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            ...
            providers: [
                { provide: LuthierService, useValue: service },
                Reflector,
            ],
        })
    ...
    describe('create()', () => {
        it('deve chamar service.create com os parâmetros corretos', async () => {
            ...
            const dto = { nomeMestre: 'João Silva', dataAbertura: new Date('2010-01-01'), certificada: true, bancadasNum: 3 };
            const result = await controller.create(dto as any);

            expect(service.create).toHaveBeenCalledWith('João Silva', new Date('2010-01-01'), true, 3);
    ...
    describe('update()', () => {
        it('deve chamar service.update com os parâmetros corretos', async () => {
            ...
            const dto = { nomeMestre: 'Carlos Mendes', dataAbertura: new Date('2010-01-01'), certificada: false, bancadasNum: 4 };
            const result = await controller.update('1', dto as any);

            expect(service.update).toHaveBeenCalledWith(1, 'Carlos Mendes', new Date('2010-01-01'), false, 4);
            expect(result).toBe(luthier);
        });
    });
