...
describe('AllExceptionFilter', () => {
    ...
    it('deve formatar BusinessException corretamente', () => {
        const exception = new BusinessException('TEST_CODE', 'Erro de negócio de teste', HttpStatus.BAD_REQUEST);
    ...
    it('deve formatar HttpException padrão corretamente', () => {
        const exception = new HttpException('Erro de requisição padrão', HttpStatus.FORBIDDEN);
    ...
    it('deve formatar HttpException de validação (BadRequest com array de mensagens) corretamente', () => {
        const exception = new HttpException(
            { message: ['campo1 deve ser email', 'campo2 é obrigatório'], error: 'Bad Request' },
