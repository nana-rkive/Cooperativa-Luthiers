...
describe('AuthController', () => {
  ...
  describe('register()', () => {
    it('deve chamar service.cadastro com os dados corretos', async () => {
      service.cadastro.mockResolvedValue({
      ...
      expect(service.cadastro).toHaveBeenCalledWith(
        'Maria',
        'Souza',
        'maria@example.com',
        '123456',
  ...
  describe('validarEmail()', () => {
    it('deve chamar service.validarEmail com o email', async () => {
      ...
      const result = await controller.validarEmail(dto);
  ...
  describe('redefinirSenha()', () => {
    it('deve chamar service.redefinirSenha com os parâmetros corretos', async () => {
      service.redefinirSenha.mockResolvedValue({
        mensagem: 'Senha atualizada.',
      });
      const dto = {
        email: 'maria@example.com',
        novaSenha: 'new123',
        confirmarNovaSenha: 'new123',
      };
      const result = await controller.redefinirSenha(dto);
      expect(service.redefinirSenha).toHaveBeenCalledWith(
        'maria@example.com',
        'new123',
        'new123',
      );
...
describe('UsuarioController', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: UsuarioService, useValue: service }, Reflector],
    })
