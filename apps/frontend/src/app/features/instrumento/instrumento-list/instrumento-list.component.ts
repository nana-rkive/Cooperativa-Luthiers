    ...
    <div class="container mx-auto p-4 max-w-6xl">
      <div class="mb-6">
        <div class="flex justify-between items-center mb-4">
          ...
          @if (authService.isLuthier()) {
            <a routerLink="/instrumentos/novo" class="bg-brand-brown-light hover:bg-opacity-95 text-white font-semibold py-2 px-5 rounded-lg shadow-sm transition-all duration-200">
              Novo Registro
            </a>
          }
        </div>
        <h1 class="text-3xl font-extrabold text-brand-brown">Instrumentos em Manutenção</h1>
      </div>
      ...
      @if (loading()) {
        <div class="flex justify-center items-center py-20">
          ...
          <span class="ml-3 text-lg text-gray-600 font-medium">Buscando instrumentos...</span>
        </div>
      } @else if (!error()) {
        ...
        @if (isEmpty()) {
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <h3 class="mt-4 text-lg font-semibold text-brand-brown">Nenhum instrumento registrado</h3>
          ...
          <div class="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-[#f3e8e2]">
                <tr>
                  <th scope="col" class="px-6 py-3.5 text-left text-xs font-bold text-brand-brown uppercase tracking-wider">Modelo / Madeira</th>
                  <th scope="col" class="px-6 py-3.5 text-left text-xs font-bold text-brand-brown uppercase tracking-wider">Luthier Responsável</th>
                  <th scope="col" class="px-6 py-3.5 text-left text-xs font-bold text-brand-brown uppercase tracking-wider">Custo (R$)</th>
                  <th scope="col" class="px-6 py-3.5 text-left text-xs font-bold text-brand-brown uppercase tracking-wider">Data Entrada</th>
                ...
                @for (item of data(); track item.id) {
                  <tr class="hover:bg-brand-offwhite hover:bg-opacity-50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm font-semibold text-gray-900">{{ item.modeloMadeira }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-700 font-medium">
                        {{ getLuthierName(item.luthierId) }}
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-600 font-medium">{{ item.custoReparo | currency:'BRL':'symbol':'1.2-2' }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      @if (item.reparoConcluido) {
                        ...
                        <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200">
                          Em Andamento
                        </span>
                      }
                    </td>
                    @if (authService.isLuthier()) {
                      <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold">
                        <a [routerLink]="['/instrumentos', item.id, 'editar']" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-bold rounded-lg shadow-sm text-amber-950 bg-amber-400 hover:bg-amber-500 mr-3 transition-colors">Editar</a>
                        <button (click)="confirmDelete(item)" class="text-red-600 hover:text-red-900 transition-colors">Excluir</button>
                      </td>
                    }
                  </tr>
                }
